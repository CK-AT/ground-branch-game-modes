...
function Mode:OnCharacterDied(Character, CharacterController, KillerController)
	...
	if goodKill then
		...
		if actor.HasTag(CharacterController, self.HVT.Tag) then
			print("HVT down, spawning reinforcements...")
			local hvtLocation = actor.GetLocation(Character)
			self.AiTeams.OpFor.Spawns:AddSpawnsFromClosestGroup(10, hvtLocation)
			self.AiTeams.OpFor.Spawns:Spawn(3.5, 10, self.AiTeams.OpFor.Tag)
		end
	end
end
